...
class AddGaussianBlur(object):
    ...
    def __call__(self, img):
        kernel_size = self.kernel_size # random.choice(self.kernel_size)
...
def main(input_folder, output_folder, num_segments=3):
    ...
    # Define blur transformations with increasing intensity
    blur_transforms = [
        (AddGaussianBlur, [3, 5, 7]),  # Different kernel sizes for increasing blur
        (AddResizingBlur, ["area", "bilinear", "bicubic"])  # Different modes for resizing blur
    ]
    ...
    for folder in sub_folders:
        ...
        for transform_class, params in blur_transforms:
            for i, param in enumerate(params):
                for segment_idx in range(i * (num_segments // len(params)), (i + 1) * (num_segments // len(params))):
                    if transform_class == AddGaussianBlur:
                        current_transform_list.append(transform_class(param))
                    elif transform_class == AddResizingBlur:
                        mode = random.choice(param)
                        current_transform_list.append(transform_class(param))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to add increasing blur to video sequences.")
